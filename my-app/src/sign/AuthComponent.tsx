...
const GitHubLogo = () => (
  <svg viewBox="0 0 24 24" className="w-5 h-5" role="img" aria-label="GitHub Logo">
...
const AuthComponent = () => {
    ...
    try {
      ...
      if (isSignUpPage) {
        if (signUp) {
          ...
          throw new Error("SignUp service not loaded");
        }
      } else {
        if (signIn) {
          await signIn.authenticateWithRedirect({
            strategy: "oauth_google",
            redirectUrl: "/language",
            redirectUrlComplete: "/language"
          });
        } else {
          throw new Error("SignIn service not loaded");
        }
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0B1120] via-[#0F172A] to-[#1E293B] flex items-center justify-center p-4">
      <motion.div
